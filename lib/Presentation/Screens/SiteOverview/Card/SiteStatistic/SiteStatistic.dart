...
import 'SiteStatisticCard.dart';

class SiteStatistic extends StatefulWidget {
  final String siteID;
  ...
  @override
  State<SiteStatistic> createState() => _SiteStatisticState();
}

class _SiteStatisticState extends State<SiteStatistic> {
  ...
  Future<Query> fetchQuery() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        machineName = [];
        for (var i = 0; i < (thisQuery.items[0])["machine_name"].length; i++) {
          machineName.add(((thisQuery.items[0])["machine_name"])[i]);
        }
        maxNum = 0;
        ...
        if (durationUnit == "days") {
          ...
          for (var j = 0; j < 7; j++) {
            List<int> unitMachineTotal = [];
            for (var i = 0;
                i < (thisQuery.items[j])["machine_total"].length;
                i++) {
              unitMachineTotal.add(((thisQuery.items[j])["machine_total"])[i]);
            }
            machineTotal.add(unitMachineTotal);
          }
          dayBarData.initializeMachineTotal(machineTotal);
          ...
          weekBarData.initializeMachineTotal(machineTotal);
        } else {
          ...
          monthBarData.initializeMachineTotal(machineTotal);
        }
      });
      return thisQuery;
    } else {
  ...
  @override
  void didUpdateWidget(covariant SiteStatistic oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.siteID != widget.siteID ||
        oldWidget.buttonChange != widget.buttonChange ||
        oldWidget.fruitType != widget.fruitType) {
      futureQuery = fetchQuery();
    }
  }
